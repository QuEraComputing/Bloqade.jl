...
from pulser.devices import MockDevice # https://pulser.readthedocs.io/en/stable/

def quspin_compare(Nqubits , Ttotal=10 , safe=False):
    """
    ...
    N = Nqubits                 # Number of atoms
    
    # Compute the radius and positions of the atoms
    radius = R_interatomic/(2*sin(pi/N * (ground_state_spacing)))
    th = linspace(0,2*pi,N+1)[0:-1]
    positions = array([radius * sin(th) , radius * cos(th)]).T
    
    #print('\nParameters of the atoms:')
    #print('Number of atoms:  {:0.0f}'.format(N))
    ...
    Trise     = 0.2*Ttotal # Rise time, in msec
    Tramp     = 0.6*Ttotal # Ramp time, in msec
    Tfall     = 0.2*Ttotal # Fall time, in msec
    ...
    ham1 = quspin.operators.hamiltonian(static,dynamic1,N=N,basis=bass,check_symm=False)
    ham2 = quspin.operators.hamiltonian(static,dynamic2,N=N,basis=bass,check_symm=False)
    ham3 = quspin.operators.hamiltonian(static,dynamic3,N=N,basis=bass,check_symm=False)
    ...
    if safe==True:
        ...
        ham = quspin.operators.hamiltonian(static,[],N=N,basis=bass,check_symm=False).tocsc()
...
if __name__=="__main__":
    # Run and time tests
    fil = open('quspin_compare.dat','w')
    nqubits_ = range(4,20)
    for nqubits in nqubits_:
        t1 = time.time()
        dout = quspin_compare(nqubits,10)
        t2 = time.time()
