...
```@example quick-start
Bloqade.plot(generate_sites(chain, 10))
```
...
Bloqade.plot(generate_sites(square, 10, 10))
```
...
Bloqade.plot(generate_sites(honeycomb, 5, 5))
```
...
Bloqade.plot(generate_sites(triangular, 8, 8))
```
...
```@example quick-start
lieb = LiebLattice()
Bloqade.plot(generate_sites(lieb, 5, 5))
```
...
Bloqade.plot(generate_sites(kagome, 5, 5))
```
...
One can check the docstring of [`Bloqade.plot`](@ref) to know more about how to custom lattice visualization.
