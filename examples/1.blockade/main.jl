...
# ```math
# V_{jk} = \frac{C_6}{|\mathbf{r}_j - \mathbf{r}_k|^6}\hat n_j \hat n_k,
# ```
...
# and distance between atoms ``|\mathbf{r}_j - \mathbf{r}_k| = 7`` μm, 
...
# and there are edges if the distance between vertices is less than or equal to some unit disk radius ``|\mathbf{r}_j - \mathbf{r}_k| \leq R_u``.
