...
#
# *Note: You will see that mc\_step\_beta!() returns three objects which can be used for computations during each MC step. "ts" stores the instantaneous SSE configuration, "h\_qmc" is the same object as before, "lsize" is related to the arrays used to the arrays used to carry out a MC step and depend on the operator sequence of the current configuration. Details are explained in the upcoming manual. Further, SSE\_slice stores a sample of the atom configuration taken from the current SSE configuration, in this case chosen to be the first vertical slice."
...
# *Note: For a detailed guide to the backend of the mc\_step\_beta!() function, please see the manual (in progress).*
