...
open("generate_schema/aws_science_tasks/global_rydberg/global5.json","w") do f
    JSON.print(f, h)
end
...
epsilon =0.05
Ω = piecewise_constant(clocks=[0.0, epsilon, T_pi_2+ epsilon, T + T_pi_2+epsilon, T + 2* T_pi_2+epsilon, T + 2* T_pi_2+2* epsilon], values= 2π*[0.0, 4, 0, 4, 0])
ϕ = piecewise_constant(clocks=[0.0, epsilon, T_pi_2+ epsilon, T + T_pi_2+epsilon, T + 2* T_pi_2+epsilon, T + 2* T_pi_2+2* epsilon], values= 2π*[0.0, 0, 0, 1/4, 0])
