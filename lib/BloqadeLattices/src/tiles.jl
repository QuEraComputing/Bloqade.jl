abstract type AbstractTile{D} end

struct Parallelepiped{D, T} <: AbstractTile{D}
    # abstract matrix dimensions already guaranteed to be 2D
    bounds_inv::AbstractMatrix{T}
end

struct Tile{D, T} <: AbstractTile{D}
    bounds_inv::AbstractMatrix{T}
end
...
# Tile{D,T} is just concrete type based off the AbstractTile{D}
# NTuple gives has D elements of type T, representing point
in_range(x) = 0 ≤ x < 1 ? true : false
within_tile(tile::Tile{D,T},x::NTuple{D,T}) where {D,T} = all(in_range.(tile.bounds_inv * collect(x)))

function wrap_around(tile::Tile{D,T},x::NTuple{D,T}) where {D,T}

end

# distance(::AbstractTile{D},x::NTuple{D,T},y::NTuple{D,T}) = 
