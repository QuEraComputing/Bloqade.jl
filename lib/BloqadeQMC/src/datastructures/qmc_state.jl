function init_op_list(length, K=Val{3}()) # I'm guessing the 3 is the default value since the LTFIM operators are defined by 3 indices?  
...
struct QMCState{S,T,K,V <: AbstractVector{T},P <: Union{Nothing, AbstractTrialState{Float64, T}}} <: AbstractQMCState{S,T,K}
    ...
    trialstate::P                                   # This is where AbstractTrialState gets used in case of GroundState simulation
