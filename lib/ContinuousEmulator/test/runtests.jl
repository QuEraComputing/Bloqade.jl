...
@testset "h=$name" for (name, h) in [
    ...
    @testset "subspace" begin
        ...
        discrete = DiscreteEvolution(ref, map(_->dt, 0.0:dt:0.2), map(h, 0.0:dt:0.2))
        emulate!(discrete)

        reg = zero_state(space, ComplexLayout())
        continuous = ContinuousEvolution(reg, 0.2, h)
        emulate!(continuous)
        @test reg ≈ ref atol=1e-4

        reg = zero_state(space, RealLayout())
        emulate!(ContinuousEvolution(reg, 0.2, h))
        @test reg ≈ ref atol=1e-4
    end

    @testset "fullspace" begin
        ref = zero_state(5)
        emulate!(DiscreteEvolution(ref, map(_->dt, 0.0:dt:0.2), map(h, 0.0:dt:0.2)))
