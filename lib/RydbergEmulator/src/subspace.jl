...
"""
    maximal_independent_set_subspace(graph)

Create a subspace from given graph's maximal independent set.
"""
function maximal_independent_set_subspace(graph::SimpleGraph)
...
function blockade_subspace(atoms::AbstractVector{<:RydAtom}, radius::AbstractFloat=1.0)
    return maximal_independent_set_subspace(unit_disk_graph(atoms, radius))
end
